...
#[wasm_bindgen]
pub fn run_webxr() -> Promise {
    let navigator: web_sys::Navigator = web_sys::window().unwrap().navigator();
    // let xr = navigator.xr().unwrap();
    let xr = navigator.xr();
    let session_mode = XrSessionMode::Inline;
    let webxr_supports_session = xr.supports_session(session_mode);
...
/*
 *Fetch example: https://rustwasm.github.io/docs/wasm-bindgen/examples/fetch.html
 */
